...
'   4. 「操作」シート B2  … テスト用マスタパス（通常は空欄。空欄=本番マスタ）
'
' 【触らなくていい箇所】
...
Public Function GetSheetSyncConfig() As Variant
    ...
    configs(5) = Array("担当者マスタ",        "A",  2, "merge",     "")    ' 全部署共通・担当者名がキー
...
' マスタファイルのパスを返す共通関数
'--------------------------------------------------------------------------------
' 【2026/5/26 改修】「操作」シート B2 セルでパスを上書きできるようにした。
'   優先順位:
'     1. 「操作」シート B2 セルにパスが入っていれば、それを最優先で使う
'        → VBE を開かずに、セルにパスを打つだけでテスト用マスタへ切替できる
'        → テスト後は B2 を空欄に戻せば、下記2の通常動作に戻る
'     2. B2 が空欄なら IS_TEST_MODE に従って TEST/PROD の定数を使う（従来動作）
'
' 【お客様がテスト用マスタで動作確認する手順】
'   ・本番マスタをコピーして例: デスクトップに「工事番号管理表_テスト.xlsm」を作る
'   ・「操作」シート B2 セルにそのコピーのフルパスを貼り付ける
'   ・「データ更新」を実行してテスト
'   ・確認できたら B2 を空欄に戻す（本番マスタに自動で戻る）
'================================================================================
Public Function GetMasterPath() As String
    ' 1. 「操作」シート B2 セルのパスを最優先で使う（空欄ならスキップ）
    Dim cellPath As String
    On Error Resume Next
    cellPath = Trim(CStr(ThisWorkbook.Sheets("操作").Range("B2").Value))
    On Error GoTo 0
    If cellPath <> "" Then
