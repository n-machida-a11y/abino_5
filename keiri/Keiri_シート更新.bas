...
'================================================================================
' [更新] Keiri_シート更新（経理用 「最新取得」ボタンの処理）
'--------------------------------------------------------------------------------
' 【何のファイル？】
'   経理用xlsm の「依頼検索」シートをマスタの最新状態で再描画するための
'   一連の処理。各部署が現場で作成した依頼書を経理側で一覧化する。
'
' 【入口】
'   マクロ「シート更新」（依頼検索シート上のボタンに紐付け）
'
' 【さわるとき】
'   依頼検索シートの列構成が変わったとき。
'================================================================================
...
'================================================================================
' モジュール: シート更新（【共有_経理用】請求書作成.xlsm 用）
'================================================================================
' 【役割】
'   依頼検索シート上で「検索実行」「上書き保存」「新規作成」ボタンが押された
'   時の処理。マスタ（工事番号管理表.xlsm）の依頼履歴シートと双方向同期する。
'
' 【主な公開マクロ】
'   マクロ_検索実行   : 依頼検索!A2 の依頼NO でマスタを検索 → 該当行を依頼検索シートに展開
'   マクロ_上書き保存 : 依頼検索シートの編集内容をマスタに上書き
'   マクロ_新規作成   : 依頼検索シートの内容を新規依頼として追加
'
' 【依頼NOの体系】
'   - 親NO       : "301" "302" ...（マスタの依頼履歴 A列の連番）
'   - 枝NO       : "301-1" "301-2" ...（同じ案件で複数回請求する場合に派生）
'   GetNextIntegerID / GetNextBranchID で採番。
'
' 【データの流れ（検索実行時）】
'   ユーザーが依頼検索!A2 に「301」等を入力 → 検索実行ボタン
'      ↓ GetMasterPath() でマスタファイルを開く
'   マスタ「依頼履歴」シートの A列で 検索値を Find
'      ↓ 該当行発見
'   TransferRowToSearchSheet で該当行のデータを依頼検索シートに展開
'      ↓
'   ユーザーが依頼検索シート上で内容を確認・編集 → 請求書作成 へ
'
' 【依頼検索シートのレイアウト】
'   A1=依頼NO ラベル, A2=依頼NO 入力欄, B2=請求書番号
'   A4-F4: ヘッダー（工事番号/請求書日付/請求宛名/工事名称/請負金額/担当者）
'   A5-F5: 案件データ
'   A6-F6: ヘッダー（工期着手/完成/引渡日/但陽信金/コメント/作成日）
'   A7-F7: 案件データ
'   A8-F8: ヘッダー（提出要項/同封物/郵便番号/郵送先住所/経理メモ/作成日）
'   A9-F9: 案件データ
'   A10  : 操作履歴
'
' 【依存関係】
...
'
' 【重要な変更履歴】
'   - 2026/5: 親NO/枝NO の採番ロジックを整理
'   - 2026/5: SyncAllDataToMaster を Function化（Boolean返却）
'   - 2026/5: GetMasterPath() ダブルクォート除去対応
'================================================================================

'================================================================================
' 設定・定数定義
' IS_TEST_MODE / SHEET_IRAI_* / GetMasterPath などは Config モジュールで一元管理。
' このモジュールでは定義しない。
'================================================================================
