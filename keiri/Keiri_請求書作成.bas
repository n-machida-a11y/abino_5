...
'================================================================================
' [請求書] Keiri_請求書作成（「請求書作成」ボタンの本体）
'--------------------------------------------------------------------------------
' 【何のファイル？】
'   選択された依頼NOから請求書テンプレート（請求書ひな形）に
'   金額・宛名・明細などを書き込み、経理用の請求書シートを生成する。
'
' 【入口】
'   依頼検索シートの「請求書作成」ボタン → AssignAccountingInvoiceNo
'
' 【しくみ】
'   - 税抜（税込/1.1）は Round() で計算（IEEE754 誤差対策）
'   - 発行日 F9 は実行時に自動入力
'================================================================================


'================================================================================
' モジュール: 請求書作成（【共有_経理用】請求書作成.xlsm 用）
'================================================================================
' 【役割】
'   依頼検索シートに表示された案件情報を、請求書テンプレートに転記して
'   新しい「No_xx_請求書」シートを生成する。
'
' 【主な公開マクロ】
'   AssignAccountingInvoiceNo  : 経理番号を採番してマスタに書き込む
'                                （請求書シート作成ではなく、番号採番だけ）
'   CreateInvoiceFromTemplate  : テンプレを複製して新しい請求書シートを作る
'                                 ※Privateなので、別マクロから呼ばれる
'
' 【テンプレ請求書シート（コピー元）】
'   - 請求書                         （通常）
'   - 請求書(振込手数料有)
...
' 【データの流れ】
'   依頼検索シート（A2に依頼NO）
'      ↓ 検索実行で取得した案件データ（B5～F5, A7～B7 等）
'   テンプレ請求書シート（請求書 等）
'      ↓ CreateInvoiceFromTemplate で複製
'   No_xx_請求書 シート（生成される）
'      ↓ TransferData で各セルに値を書き込み
'   ユーザーが内容確認 → 保存ボタン → xlsx出力
'
' 【重要なバグ修正履歴】
'   - 2026/5/20: 税抜金額（AA19）の浮動小数点誤差を Round で修正
'                旧: Int(33000000/1.1) = 29999999  ← バグ
'                新: Round(33000000/1.1) = 30000000 ← 正解
'
' 【セル位置定数の解説】
'   ＜ソース＞依頼検索シート（マスタから取得した案件情報）
'     B2 = 請求書番号(固定)
'     A5 = 工事番号
'     B5 = 請求書日付（=月年フォルダの元になる）
'     C5 = 請求書提出先（宛名）
'     D5 = 工事名称
'     E5 = 請負金額（税込）
'     F5 = 担当者
...
'     B7 = 工期完成
'
'   ＜ターゲット＞請求書シート（テンプレから複製した新シート）
'     AF1     = 経理番号
'     AB3,AE3 = 請求書日付の年・月（保存時の月年フォルダ名に使う）
'     AG3     = 請求書日付の日
'     C5:N5   = 宛名（請求書提出先）
'     O5      = 敬称（御中/様）
'     AB15:AD15 = 部署名
