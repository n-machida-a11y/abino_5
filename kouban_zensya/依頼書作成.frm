...
Private Sub 依頼書作成_Click()
    ...
    With wsRequest
        ...
        Call WriteMeisaiRow(wsRequest, 1, Me.txt名称1.Value, Me.数量1.Value, Me.単位1.Value, Me.金額1.Value)
        Call WriteMeisaiRow(wsRequest, 2, Me.txt名称2.Value, Me.数量2.Value, Me.単位2.Value, Me.金額2.Value)
        Call WriteMeisaiRow(wsRequest, 3, Me.txt名称3.Value, Me.数量3.Value, Me.単位3.Value, Me.金額3.Value)
        Call WriteMeisaiRow(wsRequest, 4, Me.txt名称4.Value, Me.数量4.Value, Me.単位4.Value, Me.金額4.Value)
        Call WriteMeisaiRow(wsRequest, 5, Me.txt名称5.Value, Me.数量5.Value, Me.単位5.Value, Me.金額5.Value)
...
Private Sub AddDataToIraiRireki(ByVal wsRireki As Worksheet)
    ...
    ' --- 2. 依頼NOを自動採番 (1, 2, 3... の通し番号) ---
    ' A列の数値の最大値+1。全部門で共通の通し番号となる（依頼履歴シートは全部門共有）。
    ' 【2026/6/3 修正】旧形式("0301"等)の互換読み取りを削除して単純化。
    '   旧互換コードは 301～399 を「"0301"の先頭ゼロ落ち」とみなして -300 していたため、
    '   通し番号が301に達すると最大値を誤認し、採番が重複するバグがあった。
    '   依頼履歴は2026/6に番号リセット済みで旧形式データは存在しないため削除してよい。
    Dim i As Long, maxNum As Long, cellVal As String
    maxNum = 0
    On Error Resume Next  ' 想定外の巨大値などでCLngが失敗しても走査を続ける
    For i = 2 To lastRow
        cellVal = Trim(CStr(wsRireki.Cells(i, "A").Value))
        If cellVal <> "" And IsNumeric(cellVal) Then
    ...
    ' --- 新規行に直上データ行の書式をコピー【2026/6/4 追加・お客様要望】 ---
    '   行追記時は書式（フォントサイズ・罫線等）がExcel既定値のままになり、
    '   「特定の行だけ文字が大きい・罫線が消える」崩れが起きていた。
    '   さらにローカル一覧への更新コピーでこの崩れが毎回持ち込まれるため、
    '   追記の時点で直上の行（既存データ行）の書式を新規行へ適用して揃える。
    '   ※最初のデータ行（直上がヘッダ行）の場合はスキップ
    If nextRow >= 3 Then
        On Error Resume Next
        wsRireki.Rows(nextRow - 1).Copy
        wsRireki.Rows(nextRow).PasteSpecial Paste:=xlPasteFormats
