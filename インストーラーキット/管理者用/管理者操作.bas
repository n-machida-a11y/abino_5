...
Public Sub CopyFilteredByDept(ByVal src As Worksheet, ByVal dst As Worksheet, _
    ...
    ' UsedRange から列範囲を、A列 End(xlUp) から行範囲を算出
    '   【2026/5/22 修正】
    '   UsedRange.Rows.Count は削除済みデータの書式痕跡を含み、実データが178行
    '   程度でも 1,048,576 まで膨らむことがある（UsedRangeゴースト）。
    '   その場合 arr = readRange.Value のメモリ確保が失敗し、
    '   「実行時エラー6: オーバーフローしました」が発生する。
    '   実データ最終行は A列の End(xlUp) で安全に判定する。
    Dim lastCol As Long
    Dim lastRow As Long
    ...
    ' ヘッダ行があり A列が空の場合に備えて、UsedRange の開始行は最低保持
