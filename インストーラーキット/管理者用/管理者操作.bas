...
Private m_DiagStep As String          ' CopyFilteredByDept 内の現在処理ステップ
Private m_DiagExtra As String         ' 追加情報（エラー時のセル位置など）
...
Cleanup:
    ...
    msg = msg & "■ 処理ステップ" & vbCrLf
    msg = msg & "  詰まった場所: " & IIf(m_DiagStep = "", "(未到達)", m_DiagStep) & vbCrLf
    If m_DiagExtra <> "" Then
        msg = msg & "  追加情報    : " & m_DiagExtra & vbCrLf
    End If
    msg = msg & vbCrLf
...
Public Sub CopyFilteredByDept(ByVal src As Worksheet, ByVal dst As Worksheet, _
    ...
    m_DiagStep = "②src.UsedRange取得"
    Dim usedRng As Range
    Set usedRng = src.UsedRange
    If usedRng Is Nothing Then Exit Sub
    If usedRng.Cells.CountLarge = 0 Then Exit Sub

    m_DiagStep = "③範囲算出(lastRow/lastCol)"
    Dim lastCol As Long
    Dim lastRow As Long
    Dim usedLastRow As Long
    lastCol = usedRng.Column + usedRng.Columns.Count - 1
    usedLastRow = usedRng.Row + usedRng.Rows.Count - 1
    ...
    Debug.Print "[CopyFilteredByDept] sheet=" & src.Name & _
                ...
                " usedLastRow=" & usedLastRow & " actualLastRow=" & lastRow & " lastCol=" & lastCol & _
                " dataStartRow=" & dataStartRow & _
                " deptFilterCol=" & deptFilterCol & " deptCode=" & deptCode

    m_DiagStep = "④readRange作成"
    Dim readRange As Range
    Set readRange = src.Range(src.Cells(1, 1), src.Cells(lastRow, lastCol))

    m_DiagStep = "⑤arr読み込み(マスタからメモリへ)"
    '   【2026/5/26 修正】一括読み込み(arr = readRange.Value)が
    '   「オーバーフローしました」で失敗するケースがあるため、
    '   失敗時は1セルずつ安全に読み込むフォールバックを追加。
    '   - 1セルずつ読めれば → そのまま正常動作（一括読み込みのクセが原因だった）
    '   - 特定セルで失敗 → m_DiagExtra にそのセル位置が残り、原因セルを特定できる
    Dim arr As Variant
    Dim rr As Long, cc As Long
    If lastRow = 1 And lastCol = 1 Then
        ReDim arr(1 To 1, 1 To 1)
        arr(1, 1) = src.Cells(1, 1).Value
    Else
        Dim bulkErr As Long
        On Error Resume Next
        arr = readRange.Value
        ...
        If bulkErr <> 0 Or Not IsArray(arr) Then
            ' 一括読み込み失敗 → 1セルずつ読み直す
            m_DiagExtra = "一括読込失敗(err=" & bulkErr & ")→セル単位で再試行"
            ReDim arr(1 To lastRow, 1 To lastCol)
            For rr = 1 To lastRow
                For cc = 1 To lastCol
                    m_DiagExtra = "セル読込中: " & rr & "行目 " & cc & "列目"
                    arr(rr, cc) = src.Cells(rr, cc).Value
                Next cc
            Next rr
            m_DiagExtra = ""
        End If
    ...
    m_DiagStep = "⑧データ行フィルタ判定"
    Dim filteredOutCount As Long: filteredOutCount = 0
    For r = dataStartRow To lastRow
        m_DiagExtra = "判定中: r=" & r & " filterColIdx=" & filterColIdx
